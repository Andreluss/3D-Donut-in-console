...
int main()
{
	...
	const float K1 = 30, K2 = 5;

	Vector3 Light(0, -1, 1);
	Light = Light.normalize();
	const float A = 6, B = 2;//katy obrotu
	float a = 0, b = 0;
	...
	for (int k = 0; ; k++, a += A, b += B) {
		...
		for (float tetha = 0; tetha < 360; tetha += tetha_step) {
			...
			for (float phi = 0; phi < 360; phi += phi_step) {
				...
				Matrix3D torus_rot({{cos(phi),    0,   sin(phi)},
									{    0,       1,      0   },
									{-sin(phi),   0,   cos(phi)}});

				//i jeszcze o k*A wokol OZ
				Matrix3D A_rot    ({{  cos(a),    0,    sin(a) },
									{ -sin(a),    1,    cos(a) },
									{    0,       0,      0    }});

				//i jeszcze o k*B wokol OX
				Matrix3D B_rot    ({{    1,      0,       0    },
									{    0,    cos(b),  sin(b)},
									{    0,   -sin(b),  cos(b)}});
				
				Vector3 pos(R2 + R1*cos(tetha), R1*sin(tetha), 0);
				Matrix3D all_rot = torus_rot * A_rot * B_rot;
				pos = pos * torus_rot;
				
				//x', y' <--- projection
				//echo(pos.x, pos.y, pos.z);
				int xp = K1 * (pos.x / (pos.z + K2));
				...
				if (pos.z < zbuff[xp + W/2][yp + H/2]) {
					...
					//obliczamy (Nx, Ny, Nz)
					Vector3 N(cos(tetha), sin(tetha), 0);
					N = N * all_rot;
					//echo(N.x, N.y, N.z);
					N = N.normalize();
...
private:
	...
	T* _V;
public:
	MappedArray(int minv=0, int maxv=-1, T value = T()) {
		_offset = -minv;
		_minv = minv;
		_maxv = maxv;
		_Vsize = maxv - minv + 1;
	...
	T& operator[] (int index) {
		return _V[index + _offset];
	}
};
template <class T>
using MA = MappedArray<T>;//jakis "alias template" to sie nazywa
*/